...
** Inline display of LaTeX outputs and plots (a port of =sage-view=)
   ...
   add the following code to your Emacs init file:
    ...
    | =sage-shell-view-latex-foreground-color= | Foreground color used in LaTeX image as string (e.g. "black", "white", "#de935f") or =nil=. If it is =nil=, then the default background color will be used.                | =nil=         |
