...
** Installation and Setup
  ...
  1. If you have Sage 7.4 or later, then add the following to your Emacs init file (=~/.emacs.d/init.el=):
     ...
     By default, =sage-shell-mode= checks if =sage-shell:use-prompt-toolkit= is correctly set when you start Sage process. You can disable this checking by setting =sage-shell:check-ipython-version-on-startup= to =nil=.
